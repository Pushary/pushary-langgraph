...
// unit-tests without the framework installed. The thin LangChain binding (the tool
// and the interrupt) lives in index.ts and builds on these functions.
...
export interface PusharyLangGraphConfig {
...
export interface PusharyAskInput {
  ...
  /** Stable node/step name, used to build a re-run-safe idempotency key. */
  readonly node?: string
  readonly type?: DecisionType
  readonly options?: readonly string[]
  readonly context?: string
  /** Present for the durable path (`pusharyInterrupt`); Pushary POSTs it when answered. */
...
// The verified, parsed callback, keyed by `correlationId`, ready to feed
...
const resolveApiKey = (config: PusharyLangGraphConfig): string => {
  const key = config.apiKey ?? process.env.PUSHARY_API_KEY
  if (!key) {
    throw new Error('Pushary: set PUSHARY_API_KEY or pass { apiKey } to the LangGraph helpers.')
  }
  return key
}

const clientFor = (config: PusharyLangGraphConfig) =>
 ...
 */
export const askExternalUser = (config: PusharyLangGraphConfig, input: PusharyAskInput): Promise<AskResult> =>
...
/**
 * Verify a callback signature and parse it, or return null. Feed `answer` into
 * `graph.invoke(new Command({ resume: answer }), { configurable: { thread_id } })`.
 */
...
export const connect = (config: PusharyLangGraphConfig, externalId: string): Promise<EnrollResult> =>
