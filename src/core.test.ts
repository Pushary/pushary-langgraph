...
describe('askExternalUser', () => {
  it('creates a decision then polls, and keys idempotency by externalId+node+question', async () => {
    ...
    const out = await askExternalUser(CONFIG, {
      question: 'Approve?',
      externalId: 'user_1',
      node: 'approval',
    })
...
describe('createDurableDecision', () => {
  it('opens a non-waiting decision with the callback and returns the correlationId', async () => {
    const calls = installFetch([
      () => ({ decisionId: 'd2', status: 'pending', reachable: true, reachableChannels: 1 }),
    ])
...
describe('describeAnswer', () => {
  it('formats every outcome unambiguously', () => {
...
describe('resolvePusharyCallback', () => {
  it('verifies, parses, and folds a fail-closed approved flag', () => {
    const body = JSON.stringify({ correlationId: 'd1', answer: 'yes', answeredAt: '', context: 't-42' })
    const out = resolvePusharyCallback(body, sign(body), SECRET)
    expect(out?.correlationId).toBe('d1')
    expect(out?.approved).toBe(true)
    expect(out?.context).toBe('t-42')
  })
